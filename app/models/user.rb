class User < ApplicationRecord
  ...
  scope :search, ->(search) {(search.nil? ? all : where(approved: true).where('lower(name) LIKE lower(?) OR lower(emp_id) LIKE lower(?)', "%#{search}%", "%#{search}%") )}
  scope :users_list, -> { where(approved: true).order(:emp_id) }   
